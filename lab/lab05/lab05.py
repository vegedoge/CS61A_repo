...
def berry_finder(t):
    ...
    if not is_tree(t):
        ...
        result = [berry_finder(x) for x in branches(t)]
...
def riffle(deck):
    ...
    """
    "*** YOUR CODE HERE ***"
    return [deck[M//2 + M%2 * len(deck)//2] for M in range(len(deck))]
...
def add_trees(t1, t2):
    ...
    zip_tree = list(zip(t1, t2))
    if len(zip_tree) > 1:
        added_branches = [add_trees(zip_tree[i][0], zip_tree[i][1]) 
                          for i in range(1, len(zip_tree))]
    else:
        added_branches = []
    added_tree = tree(sum(zip_tree[0]), added_branches)
    return added_tree
